...
| `.config/zsh/banner.zsh` | Startup banner, modelled on Nushell's. Disable with `DOTFILES_BANNER=0`; run `motd` to reprint, `motd --short` for startup time only. Caches the OS version and the `doctor.sh` summary under `~/.cache/dotfiles` — the latter refreshed by a disowned background job (24h), never inline. |
